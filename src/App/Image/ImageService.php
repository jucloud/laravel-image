...
use App\Image\Exception\StoreImageException;
use App\Image\Models\ImageHash;
...
class ImageService
{
    ...
    {
        return config("image.$key", $default);
    }
    ...
    public function storeImageFromPath($path_with_name, $file_extension = null, $isAllowGIF = false)
    {
        ...
        if ($isExists) {
            ...
            $image_hash = ImageHash::where(['file_sha1' => $file_sha1])->first();
            $isExists = $isSimilarExists = $image_hash ? true : false;
        }
        ...
        $image = null;
        if ($isExists) {
            $image = $this->saveImageInfo($isSimilarExists, $image_hash,
    ...
    public function storeImageFromURLEncodedImageData($originImageEncodedData)
    {
        ...
        $origin_image_data = ImageService::convertDataURLEncodedToImageData($originImageEncodedData);
    ...
    public function storeImageFromMake($source, $file_sha1, $exif)
    {
        try {
            ...
            throw new StoreImageException("ImageService: Unable to make image [" + (string)$e + "]");
        }
    ...
    {
        return Image::where('image_name', $image_name)->firstOrFail();
    }
    ...
    public function showImage($template_name, $image_name, array $image_templates = [], $storage_path = null)
    {
        ...
        if (!isset($image_templates[$template_name])) {
            throw new ImageException('Invalid template name supplied.');
        }
     ...
     * @return Image
     */
    private function saveImageInfo($isSimilarExists, $image_hash, $final_file_name, $is_allowed_animated_gif, $final_file_size, $image, $exif, $file_sha1, $final_file_sha1)
    {
        // check has image
        if ($isSimilarExists) {
            $image = $image_hash->image;
        } else {
            $image = Image::where('image_name', $final_file_name)->first();
        }
    ...
    public static function convertDataURLEncodedToImageData($encodedData)
    {
        try {
            ...
            \Log::error("ImageService: Unable to convert to image [" + (string)$e + "]");
            return null;
        }
    }
     ...
     */
    public static function getImageFromImageId($image_id)
    {
        $image = Image::find($image_id);
        if (is_null($image)) {
            throw new ImageException('Image record not found!');
        }
        $imageManager = new ImageManager(config('image'));
        $background_image = $imageManager->make($image->getImagePath());
